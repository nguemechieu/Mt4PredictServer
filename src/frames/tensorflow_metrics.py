...
class TensorFlowMetricsTab(QWidget):
    def __init__(self):
        ...
        self.TENSORBOARD_LOG_DIR = Path("src/logs/tensorboard")
        os.makedirs(self.TENSORBOARD_LOG_DIR, exist_ok=True)
        os.makedirs(self.TENSORBOARD_LOG_DIR / "models", exist_ok=True)
    ...
    def show_tensorboard_metrics(self, tensorboard_log_dir=None):
        ...
        if not any(log_dir.glob("events.*")):
            msg = f"⚠️ No TensorBoard event files found in: {log_dir}"
            logging.warning(msg)
            self.metrics_tab = self._make_textbox(msg)
            self.layout.addWidget(self.metrics_tab)
            return
        ...
        try:
            ...
            if scalar_tags:
                self.metrics_tab = PredictionChart(controller=self)
    ...
    # Utility
    # ====================================================
    def _make_textbox(self, text: str = None) -> QTextEdit:
        """Helper to create a readonly text box with text."""
        box = QTextEdit()
