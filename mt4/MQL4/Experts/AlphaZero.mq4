...
#property copyright "Copyright 2021-2025, SOPOTEK ,LLC."
#property link      "https://github.com/nguemechieu/alphazero"
#property version   "1.0"
#property strict
#resource "\\Indicators\\ZigZag.ex4"
#resource "\\Indicators\\RSI.ex4"
#resource "\\Indicators\\ATR.ex4"
#resource "\\Indicators\\Momentum.ex4"
#resource "\\Indicators\\MACD.ex4"
#resource "\\Indicators\\Stochastic.ex4"
#resource "\\Indicators\\CCI.ex4"
#resource "\\Indicators\\Bears.ex4"
#resource "\\Indicators\\Bulls.ex4"
...
void CheckCommands() {
   for (int i=0; i<maxCommandFiles; i++) {
      ...
      if (command == "OPEN_ORDER") {
         ...
         ResetCommandIDs();
      } else if(command=="CLOSE_SELL_ORDERS_BY_SYMBOL"){
         
        for(i=0;i<SymbolsTotal(false);i++){
        
        if( content == SymbolName(i,true)){
        int clo= OrderClose(OrderTicket(),OrderLots(),0,0,clrAliceBlue);
        SendError( (string)GetLastError(),ErrorDescription(clo));
        }  
       }
      }
      else if(command=="CLOSE_BUY_ORDERS_BY_SYMBOL"){
      for(i=0;i<SymbolsTotal(false);i++){
    ...
    }else if(command=="SUBSCRIBE_SIGNALS"){
        
         SubscribeSignal(content);
...
int signal;
int generateSignal(string symbol) {
    double zzCurr = iCustom(symbol, 0, "ZigZag.ex4", 0, 1);
    double zzPrev = iCustom(symbol, 0, "ZigZag.ex4", 0, 2);
    double rsi = iCustom(symbol, 0, "RSI.ex4", 0, 0);
    double macdMain = iCustom(symbol, 0, "MACD.ex4", 0, 0);
    double macdSignal = iCustom(symbol, 0, "MACD.ex4", 1, 0);
    bool rsiBuy = (rsi < RSI_Oversold);
    bool rsiSell = (rsi > RSI_Overbought);
    bool macdBuy = (macdMain > macdSignal);
    bool macdSell = (macdMain < macdSignal);

    if (zzCurr < zzPrev) {
        if ((!UseRSIConfirmation || rsiBuy) && (!UseMACDConfirmation || macdBuy)) {
            signal = 1;
        }
    } else if (zzCurr > zzPrev) {
        if ((!UseRSIConfirmation || rsiSell) && (!UseMACDConfirmation || macdSell)) {
            signal = -1;
        }
    }

    // Log features to DWX_Signals.txt
    string logLine = StringFormat(
        "{\"symbol\":\"%s\",\"time\":\"%s\",\"zzCurr\":%.5f,\"zzPrev\":%.5f,\"rsi\":%.2f,\"macdMain\":%.5f,\"macdSignal\":%.5f,\"signal\":%d}",
        symbol,
        TimeToString(TimeCurrent(), TIME_DATE | TIME_SECONDS),
        zzCurr, zzPrev, rsi, macdMain, macdSignal, signal
    );

    int handle = FileOpen(filePathSignals, FILE_WRITE | FILE_TXT | FILE_COMMON);
    if (handle != INVALID_HANDLE) {
        FileSeek(handle, 0, SEEK_END);
        FileWrite(handle, logLine);
        FileClose(handle);
    } else {
        Print("Error writing to DWX_Signals.txt: ", ErrorDescription(GetLastError()));
    }
...
string filePathSignals=folderName+"/DWX_Signals.txt";

void   SubscribeSignal(string symbol){
 signal= generateSignal(symbol);

string  messages = StringFormat("{\"SIGNAL\": \"%n\"}", signal);
   SendInfo(messages);
}
void OpenOrder(string orderStr) {
   ...
   if ( orderType == OP_BUYLIMIT|| orderType == OP_BUYSTOP ) price = MarketInfo(symbol, MODE_ASK);
   if ( orderType == OP_SELLLIMIT|| orderType == OP_SELLSTOP) price = MarketInfo(symbol, MODE_BID);
...
void GetHistoricData(string dataStr) {
    ...
    if (StringLen(symbol) == 0) {
        SendError("HISTORIC_DATA_SYMBOL", "Empty symbol in: " + dataStr);
        return;
    }

    if (!SymbolSelect(symbol, true)) {
        SendError("HISTORIC_DATA_SELECT_SYMBOL", "Failed to select symbol " + symbol + ". Error: " + ErrorDescription(GetLastError()));
        return;
    }

    if (openChartsForHistoricData && OpenChartIfNotOpen(symbol, timeFrame)) {
        Sleep(200);  // Give time to load chart
    }

    MqlRates rates_array[];
    ArraySetAsSeries(rates_array, true);

    int rates_count = CopyRates(symbol, timeFrame, dateStart, dateEnd, rates_array);
    
    for (int i = 0; i < 10 && rates_count <= 0; i++) {
        int errorCode = GetLastError();
        if (errorCode != 4066 && errorCode != 4073) break;
        Sleep(200);
        rates_count = CopyRates(symbol, timeFrame, dateStart, dateEnd, rates_array);
    }

    if (rates_count <= 0) {
        SendError("HISTORIC_DATA", "Failed to get historic data for " + symbol + "_" + data[1] + ": " + ErrorDescription(GetLastError()));
        return;
    }

    // Build JSON response
    string json = "{\"" + symbol + "_" + TimeFrameToString(timeFrame) + "\": {";
    bool first = true;

    for (int i = 0; i < rates_count; i++) {
        if (first) {
            double daysDiff = MathAbs(rates_array[i].time - dateStart) / 86400.0;
            if ((timeFrame == PERIOD_MN1 && daysDiff > 33) ||
                (timeFrame == PERIOD_W1 && daysDiff > 10) ||
                (timeFrame < PERIOD_W1 && daysDiff > 3)) {
                SendInfo(StringFormat("%s@%s: Returned data starts %.1f days after requested start. Data may be incomplete.", symbol, TimeFrameToString(timeFrame), daysDiff));
            }
            Print(dateStart, " | ", rates_array[i].time, " | ", daysDiff);
        } else {
            json += ", ";
        }

        json += StringFormat(
            "\"%s\": {\"open\": %.5f, \"high\": %.5f, \"low\": %.5f, \"close\": %.5f, \"tick_volume\": %.0f}",
    ...
    // Attempt to write JSON to file up to 5 times
    for (int i = 0; i < 5; i++) {
        if (WriteToFile(filePathHistoricData, json)) break;
        Sleep(100);
    }
...
bool WriteToFile(string filePath, string text) {
   ...
   if (handle == -1){
   SendInfo("ERROR OPENING "+filePathHistoricData+"--"+ErrorDescription(GetLastError()));
